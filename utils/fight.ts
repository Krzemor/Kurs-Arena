...
export const fight = (warrior1: WarriorRecord, warrior2: WarriorRecord): {
    ...
    do {
        ...
        if (defender.dp + defender.warrior.agility > attackStrength) {
            ...
            if  (defender.dp < 0) {
                log.push(`${attacker.warrior.name} przełamal obronę ${defender.warrior.name} zadając mu ${-defender.dp} obrażeń`);
                defender.hp += defender.dp;
            }
        } else {
            log.push(`${attacker.warrior.name} zadał ${attackStrength} odrażeń ${defender.warrior.name}`);
